
#include <iostream>
#include <chrono>
#include <thread>
#include "../../ffmpeg-cpp/ffmpegcpp.h"

using namespace std;
using namespace ffmpegcpp;

int main()
{
	// This example will apply some filters to a video and write it back.
	try
	{
		cout << "Creating muxing object" << endl;

		// Create a muxer that will output the video as MKV.
		Muxer *muxer = new Muxer("audio_merged.opus");
		cout << "Creating AudioCodec object" << endl;
		// Create an MP3 codec that will encode the raw data.
		AudioCodec *codec = new AudioCodec(AV_CODEC_ID_OPUS);
		cout << "Creating AudioEncoder object" << endl;
		// Create an encoder that will encode the raw audio data as MP3.
		// Tie it to the muxer so it will be written to the file.
		AudioEncoder *encoder = new AudioEncoder(codec, muxer);
		// cout<<"Creating filter object"<<endl;
		// Create a video filter and do some funny stuff with the video data.
		cout << "Creating Filter object" << endl;
		Filter *delay_filter = new Filter("adelay",encoder);
		
		// Filter *amix_filter = new Filter("amix", encoder);

		// THIS COMMENTED EXAMPLE BELOW SHOWS A FILTER FROM AUDIO TO PNG
		// It will render the waveform of the audio.

		// Create a muxer that will output as PNG.
		/*Muxer* muxer = new Muxer("Vivaldi_waveform.png");

		// Create a MPEG2 codec that will encode the raw data.
		VideoCodec* codec = new PNGCodec();

		// Create an encoder that will encode the raw audio data as MP3.
		// Tie it to the muxer so it will be written to the file.
		VideoEncoder* encoder = new VideoEncoder(codec, muxer);

		Filter* filter = new Filter("showwavespic=colors=white:s=512x64", encoder); */

		// Load a video from a container and send it to the filter first.
		cout << "Creating Demuxer object" << endl;
		Demuxer *demuxer = new Demuxer("../samples/1.opus");
		Demuxer *demuxer_1 = new Demuxer("../samples/2.opus");
		Demuxer *demuxer_2 = new Demuxer("../samples/3.opus");
		cout << "Creating BestAudioStream" << endl;
		demuxer->DecodeBestAudioStream(delay_filter);
		// demuxer_1->DecodeBestAudioStream(delay_filter);
		// demuxer_2->DecodeBestAudioStream(delay_filter);
		// demuxer->DecodeBestAudioStream(amix_filter);
		cout << "Preparing Pipeline" << endl;
		// Prepare the output pipeline. This will push a small amount of frames to the file sink until it IsPrimed returns true.
		demuxer->PreparePipeline();
		cout << "Inside PipeLine" << endl;
		// Push all the remaining frames through.
		while (!demuxer->IsDone())
		{
			// cout << "pushing frames" << endl;
			demuxer->Step();
		}

		// Save everything to disk by closing the muxer.
		cout << "closing muxer" << endl;
		muxer->Close();
	}
	
	catch (FFmpegException e)
	{
		cerr << "Exception caught!" << endl;
		cerr << e.what() << endl;
		throw e;
	}

	cout << "Encoding auto complete!" << endl;
	cout << "Press any key to continue..." << endl;

	// getchar();
}
